...
### v1现有AI架构继承
基于v1成熟的AI模型管理体系：
- **3个AI模型**: Star(gpt-4o-mini)、StarPlus(gpt-4o)、StarCode(gpt-4.1)
...
# 计划中的代码模型系列扩展
FUTURE_CODE_MODELS = {
    "StarCode": "gpt-4.1",         # ✅ 当前已有
    "StarCodeMini": "gpt-4.1-mini", # 🔄 计划新增 - 轻量级代码模型
    "StarCodeNano": "gpt-4.1-nano", # 🔄 计划新增 - 超轻量代码模型
}

# 扩展后的模型特性对比
MODEL_COMPARISON = {
    "StarCode": {"tokens": 4000, "complexity": "高", "cost": "中等"},
    "StarCodeMini": {"tokens": 2000, "complexity": "中", "cost": "低"},
    "StarCodeNano": {"tokens": 1000, "complexity": "轻量", "cost": "极低"},
}
```
...
#### rag_similarity_threshold = 0.7 详细考量
...
def analyze_similarity_distribution():
    """
    0.9-1.0: 极高相关(8%)  - 几乎完全匹配，质量极佳但数量稀少
    0.8-0.9: 高相关(12%)   - 主题完全一致，高质量答案  
    0.7-0.8: 中等相关(25%) - 有用信息，值得参考，平衡点⭐
    0.6-0.7: 低相关(30%)   - 可能相关，但包含噪声
    0.0-0.6: 不相关(25%)   - 噪音数据，严重干扰AI回答
    """
    
    # 0.7是经验最佳平衡点：
    # ✅ 保留45%高质量内容(0.7以上) - 确保回答准确性
    # ❌ 过滤55%低质量内容(0.7以下) - 避免噪音干扰
    return 0.7
```

**实际应用场景对比**：
```python
# 场景1: 用户问"Python装饰器怎么用？"
raw_search_results = [
    (0.92, "Python装饰器的定义和使用方法详解..."),    # ✅ 直接相关
    (0.84, "Python高级特性：装饰器应用实例..."),      # ✅ 高质量扩展
    (0.76, "函数式编程概念与装饰器模式..."),         # ✅ 相关背景知识
    (0.68, "Python面向对象编程基础教程..."),        # ❌ 主题偏离
    (0.45, "Java注解与Python装饰器对比..."),        # ❌ 混淆概念
    (0.32, "Web开发框架中的中间件设计..."),         # ❌ 完全无关
]

# 使用0.7阈值过滤后：
filtered_results = [(0.92, ...), (0.84, ...), (0.76, ...)]  # 3个高质量结果
# AI回答质量：准确、相关、有深度

# 如果使用0.5阈值（较低）：
low_threshold_results = [...6个结果，包含噪音...]
# AI回答问题：信息冗余、主题偏离、用户困惑

# 如果使用0.9阈值（较高）：  
high_threshold_results = [(0.92, ...)]  # 仅1个结果
# AI回答问题：信息过少、缺乏深度、遗漏重要知识点
```

**0.7阈值的具体优势**：
1. **质量保证**: 过滤掉60%的噪音数据，确保相关性
2. **数量平衡**: 通常返回3-5个高质量片段，信息充足但不冗余  
3. **成本控制**: 减少无效token消耗，降低API调用成本
4. **用户体验**: 回答准确性高，不会答非所问

### 模型管理系统
- **模型抽象**: 自定义模型名称到OpenAI模型的映射
- **配置管理**: 模型参数、成本、限制的统一配置
- **降级机制**: OpenAI API失败时的Mock备用机制
- **版本管理**: 模型版本跟踪和兼容性管理
...
class AIModelConfig(Base):
    ...
    max_context_length = Column(Integer, default=128000)  # 最大上下文长度
    temperature_default = Column(DECIMAL(3, 2), default=0.7)
    temperature_min = Column(DECIMAL(3, 2), default=0.0)
    temperature_max = Column(DECIMAL(3, 2), default=2.0)
    ...
    replacement_model = Column(String(50), nullable=True)  # 替代模型
    
    # === 使用限制 ===
    daily_request_limit = Column(Integer, nullable=True)  # 每日请求限制
    user_access_level = Column(String(20), default="all")  # all, premium, admin
    ...
    # === 关系定义 ===
    usage_logs = relationship("AIUsageLog", back_populates="model_config")
```

### 3. AIUsageLog 模型（AI使用日志）
```python
class AIUsageLog(Base):
    ...
    # === 请求信息 ===
    request_type = Column(String(20), nullable=False)  # generate, title, summary
    model_name = Column(String(50), nullable=False)  # Star, StarPlus, StarCode
    openai_model = Column(String(100), nullable=False)  # 实际调用的OpenAI模型
    ...
    # === 成本信息 ===
    input_cost = Column(DECIMAL(20, 8), nullable=False)
    output_cost = Column(DECIMAL(20, 8), nullable=False)
    total_cost = Column(DECIMAL(20, 8), nullable=False)
    
    # === 性能指标 ===
    response_time_ms = Column(Integer, nullable=False)
    queue_time_ms = Column(Integer, nullable=True)  # 队列等待时间
    
    # === 请求上下文 ===
    context_size = Column(Integer, nullable=True)  # 上下文长度
    rag_sources_count = Column(Integer, default=0)  # RAG源数量
    ...
    # === 复合索引 ===
    __table_args__ = (
        Index('idx_user_date', user_id, func.date(created_at)),
        Index('idx_model_usage', model_config_id, created_at),
        Index('idx_cost_analysis', total_cost, created_at),
    )
    
    # === 关系定义 ===
    user = relationship("User", back_populates="ai_usage_logs")
    chat = relationship("Chat", back_populates="ai_usage_logs")
    message = relationship("Message", back_populates="ai_usage_log")
    model_config = relationship("AIModelConfig", back_populates="usage_logs")
```

### 4. RAGCollection 模型（向量集合管理）
```python
class RAGCollection(Base):
    ...
    collection_type = Column(String(20), nullable=False, index=True)  # course, global, personal
    
    # === 关联信息 ===
    course_id = Column(Integer, ForeignKey("courses.id"), nullable=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=True, index=True)  # personal类型时使用
    ...
    avg_response_time_ms = Column(Integer, nullable=True)  # 平均响应时间
    
    # === 时间戳 ===
    created_at = Column(DateTime, server_default=func.now())
    
    # === 关系定义 ===
    course = relationship("Course", back_populates="rag_collections")
