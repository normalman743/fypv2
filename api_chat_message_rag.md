...
### 功能说明
- **file_ids**: 直接指定要作为上下文的文件ID列表
- **folder_ids**: 指定文件夹ID列表，系统会自动获取文件夹中的所有文件
- **智能合并**: 系统会将folder_ids中的所有文件与file_ids合并，并自动去重
- **双重上下文**: AI会同时使用RAG检索结果和用户指定的文件内容
- **优先级**: 用户指定的文件内容优先级高于RAG检索结果

### 使用场景
1. **精准引用**: 用户想要AI基于特定文件回答问题
2. **文件夹分析**: 用户需要AI分析整个文件夹的内容
3. **混合使用**: 同时使用特定文件和文件夹内容作为上下文

### 权限控制
- 用户只能引用自己拥有的课程中的文件和文件夹
- 系统会验证用户对所有指定文件和文件夹的访问权限
- 全局文件无需权限验证
